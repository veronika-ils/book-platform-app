...
<div class="done">
    <div class="books">
     <router-link to="/1984">
        <img src="../books/book1.jpg" alt="1984" class="book">
     <div class="content">
       <h1>1984</h1>
       <p>
        by George Orwell
...
export default {
    ...
    created() {
        ...
        const readKey = `book1_${userId}`;
        const isRead = Cookies.get(readKey);
       
             const cookieKey = `isFavourite1_${userId}`;
             const isFave = Cookies.get(cookieKey);
    
            if(isFave !== undefined ){
                this.isFavourite1 = JSON.parse(isFave);
                this.image = this.isFavourite1 ? require('../outline1.png') : require('../outline.png');
                this.selected = this.isFavourite1 ? 2 : 1;
            }
            if(isRead === 'Read') {
                this.read = 'true';
            }
    ...
    methods:{
        ...
        const userId = this.getUserId();
        const cookieKey = `isFavourite1_${userId}`;
        Cookies.set(cookieKey,JSON.stringify(this.isFavourite1),{expires: 7});
