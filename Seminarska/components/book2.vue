...
<div class="done">
    <div class="books">
    <router-link to="/ToKillAMockingBird"><img src="../books/book2.jpg" alt=" To Kill a Mockingbird" class="book">
    <div class="content">
...
export default {
    ...
    created() {
        ...
        const readKey = `book2_${userId}`;
        const isRead = Cookies.get(readKey);
        const cookieKey = `isFavourite2_${userId}`;

        const isFave = Cookies.get(cookieKey);
        
    
            if(isFave !== undefined ){
                this.isFavourite2 = JSON.parse(isFave);
                this.image = this.isFavourite2 ? require('../outline1.png') : require('../outline.png');
                this.selected = this.isFavourite2 ? 2 : 1;
            }
            if(isRead === 'Read') {
                this.read = 'true';
            }
    ...
    methods:{
        ...
        const userId = this.getUserId();
        const cookieKey = `isFavourite2_${userId}`;
        Cookies.set(cookieKey,JSON.stringify(this.isFavourite2),{expires: 7});
